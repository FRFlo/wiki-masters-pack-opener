...
export class WikiMasters {
	...
	private async request(path: string, init: RequestInit = {}) {
		...
		if (!response.ok)
			throw new Error(
				`Wiki Masters HTTP ${response.status}: ${data?.error || text || response.statusText}`,
			);
		return data;
	}
	openPack() {
	...
	bid(id: string, amount: number) {
		return this.request(`/marketplace/${encodeURIComponent(id)}/bid`, {
			method: "POST",
	...
	mine() {
		return this.request("/marketplace/mine");
	}
	collection(page = 0, limit = 50, pending = false) {
		return this.request(
			`/my-collection?page=${page}&limit=${limit}&sort=rarity${pending ? "&pending=1" : ""}`,
		);
	}
	sales(cardId: string) {
	...
	cancelAuction(id: string) {
		return this.request(`/marketplace/${encodeURIComponent(id)}`, { method: "DELETE" });
	}
	trade(id: string) {
		return this.request(`/trades/${encodeURIComponent(id)}`);
	}
	...
	async monthlyViews(title: string) {
		...
		const r = await fetch(
			`https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/fr.wikipedia/all-access/user/${enc}/monthly/${start.toISOString().slice(0, 10).replaceAll("-", "")}/${end.toISOString().slice(0, 10).replaceAll("-", "")}`,
	...
	async supabase(path: string, init: RequestInit = {}) {
		const response = await fetch(`${SUPABASE_URL}/${path}`, {
			...
			headers: {
				apikey: SUPABASE_KEY,
				Authorization: `Bearer ${this.supabaseToken() || SUPABASE_KEY}`,
				Accept: "application/json",
				...(init.headers || {}),
			},
			signal: AbortSignal.timeout(30_000),
		...
		if (!response.ok)
			throw new Error(`Supabase HTTP ${response.status}: ${data?.message || text}`);
		return data;
	}
	private supabaseToken() {
		...
		try {
			...
			return parsed?.access_token || (Array.isArray(parsed) ? parsed[0] : "");
		} catch {
	...
	userTags(userId = this.userId()) {
		return this.supabase(
			`tags?user_id=eq.${encodeURIComponent(userId)}&select=id,name&order=name.asc`,
		);
	}
	createTag(userId: string, name: string) {
		return this.supabase("tags", {
			method: "POST",
			headers: { "Content-Type": "application/json", Prefer: "return=representation" },
			body: JSON.stringify({ user_id: userId, name }),
		});
	}
	tagCards(userCardIds: string[], tagId: string) {
		return this.supabase(
			"user_card_tags?on_conflict=user_card_id%2Ctag_id&columns=%22user_card_id%22,%22tag_id%22",
			{
				method: "POST",
				headers: {
				...
				body: JSON.stringify(
					userCardIds.map((user_card_id) => ({ user_card_id, tag_id: tagId })),
				),
			},
		);
	}
